    ...
    series: [
        {
            name: 'pH Indicator',
            type: 'gauge',
            detail: {formatter:'{value} pH'},
            axisLine: {     
                lineStyle: {       
                    color: [[0.2, '#556ee6'],[0.8, '#34c38f'],[1, '#f46a6a']], 
                    width: 20
                }
            },
            data: [{ value: 0, }],
...
const getPH = async function (id) {
    const request = await fetch(`http://127.0.0.1:8000/api/data/${id}`)
    const value = await request.json();
    let pH = value[value.length - 1].ammonia

    option.series[0].data[0].value = pH
    myChart.setOption(option, true);  
}
    ...
    series: [
        {
            ...
            axisLine: {     
                lineStyle: {       
                    color: [[0.2, '#556ee6'],[0.5, '#34c38f'],[1, '#f46a6a']], 
                    width: 20
                }
            },
            data: [{value: 25,}]
